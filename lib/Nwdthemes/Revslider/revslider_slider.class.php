<?php
	...
	class RevSlider extends UniteElementsBaseRev{
		...
		const DEFAULT_POST_SORTBY = "position";
		const DEFAULT_POST_SORTDIR = "ASC";
		...
		public function getShortcode(){
			$shortCode = '{{revslider alias="' . $this->alias . '"}}';
			return($shortCode);
		}
		...
		private function validateInputSettings($title,$alias,$params){
			...
			$templateID =  UniteFunctionsRev::getVal($params, "slider_template_id");

			if(($sourceType == "posts" || $sourceType == 'specific_posts') && empty($templateID))
		...
		private function duplicateSlider(){
			...
			$newSlider = $this->db->fetchSingle(GlobalsRevSlider::$table_sliders, "id={$this->id}");

			unset($newSlider['id']);

			$newSlider["title"] = $newSliderTitle;
			$newSlider["alias"] = $newSliderAlias;

			$params = $this->arrParams;
			$params["title"] = $newSliderTitle;
			$params["alias"] = $newSliderAlias;
			$jsonParams = json_encode($params);
			$newSlider["params"] = $jsonParams;

			$lastID = $this->db->insert(GlobalsRevSlider::$table_sliders, $newSlider);

			//duplicate slides
			
			Mage::getModel('nwdrevslider/sliders')->duplicateSlides($this->id, $lastID);
		...
		public function duplicateSlide($slideID){
			...
			$lastID = Mage::getModel('nwdrevslider/slides')->duplicate($slideID);
		...
		private function shiftOrder($fromOrder){
			Mage::getModel('nwdrevslider/sliders')->shiftOrder($this->id, $fromOrder);
		}
		 ...
		 * TODO: make sure static css added to export
		 */
        ...
        public function importSliderFromPost($updateAnim = true, $updateStatic = true, $exactfilepath = false){
			...
			try{
				...
				$importZip = class_exists("ZipArchive");

				// Added by ThemeFuzz ( Stefan )
				if ( $importZip !== false  )
				{
					if($exactfilepath === false) {
						$new_path = Mage::getBaseDir('tmp') . DS . $_FILES['import_file']['name'];
						move_uploaded_file($filepath, $new_path);
						$filepath = $new_path;
					}
				...
				if($importZip === true){ //we have a zip
					...
					if(!empty($animations)){
						...
						Mage::helper('nwdrevslider')->dmp(Mage::helper('nwdrevslider')->__("animations imported!"));
					}else{
						Mage::helper('nwdrevslider')->dmp(Mage::helper('nwdrevslider')->__("no custom animations found, if slider uses custom animations, the provided export may be broken..."));
					}
					...
					if(is_array($dynamicCss) && $dynamicCss !== false && count($dynamicCss) > 0){
						...
						Mage::helper('nwdrevslider')->dmp(Mage::helper('nwdrevslider')->__("dynamic styles imported!"));
					}else{
						Mage::helper('nwdrevslider')->dmp(Mage::helper('nwdrevslider')->__("no dynamic styles found, if slider uses dynamic styles, the provided export may be broken..."));
					}
				}
				...
				if(isset($sliderParams["background_image"]))
					$sliderParams["background_image"] = Mage::helper('nwdrevslider')->getImageUrlFromPath($sliderParams["background_image"]);
				...
				if($sliderExists){
					...
					while ($this->isAliasExistsInDB($arrInsert["alias"])) {
						$arrInsert["alias"] .= '1';
					}
				...
				foreach($arrSlides as $slide){
						...
						if(strpos($params["image"], 'http') !== false){
							...
							if(trim($params["image"]) !== ''){
								if($importZip === true){ //we have a zip, check if exists
									...
									if(!$image){
										echo $params["image"].Mage::helper('nwdrevslider')->__(' not found!<br>');

									}else{
										if(!isset($alreadyImported['zip://'.$filepath."#".'images/'.$params["image"]])){
											$importImage = Mage::helper('nwdrevslider')->importMedia('zip://'.$filepath."#".'images/'.$params["image"], $sliderParams["alias"].'/');
							...
							$params["image"] = Mage::helper('nwdrevslider')->getImageUrlFromPath($params["image"]);
						}
					}
					...
					foreach($layers as $key=>$layer){
						if(isset($layer["image_url"])){
							...
							if(trim($layer["image_url"]) !== ''){
								if(strpos($layer["image_url"], 'http') !== false){
									...
									if($importZip === true){ //we have a zip, check if exists
										...
										if(!$image_url){
											echo $layer["image_url"].Mage::helper('nwdrevslider')->__(' not found!<br>');
										}else{
											if(!isset($alreadyImported['zip://'.$filepath."#".'images/'.$layer["image_url"]])){
												$importImage = Mage::helper('nwdrevslider')->importMedia('zip://'.$filepath."#".'images/'.$layer["image_url"], $sliderParams["alias"].'/');
							...
							$layer["image_url"] = Mage::helper('nwdrevslider')->getImageUrlFromPath($layer["image_url"]);
							$layers[$key] = $layer;
						}
					}
		...
		/**
		 * Update sliders param value
		 *
		 * @param $key
		 * @param $value
		 */

		public function updateParamValue($key, $value) {
			$this->updateParam(array($key => $value));
		}
			...
			if($isTemplate == "true"){
				...
				if(!empty($arrSliders)){
					foreach($arrSliders as $slider){
						if($slider->isSlidesFromPosts() && $slider->getParam("slider_template_id",false) === $sliderID){
		...
		public function updateSlidesOrderFromData($data){
			...
			if ($this->isSlidesFromPosts())
			{
				$arrUpdate = array('posts_list' => implode(',', $arrIDs));
		 ...
		 */
		public function getSlidesForOutput($publishedOnly = false, $storeID = 0){
		...
		public function getMaxOrder(){
			$this->validateInited();
			$maxOrder = Mage::getModel('nwdrevslider/sliders')->getMaxOrder($this->id);
			return($maxOrder);
		}
		...
		private function getPostsFromCategoies($publishedOnly = false){
			...
			$arrCatIDs = explode(',', $catIDs);
			foreach ($arrCatIDs as &$_catID) {
				$_catID = trim(str_replace('category_', '', $_catID));
			}
			unset($_catID);
			$catIDs = implode(',', $arrCatIDs);
			...
			$arrPosts = Mage::helper('nwdrevslider')->getProductsByCategoryIDs($catIDs, $sortBy, $sortDir, $maxPosts);
		...
		private function getPostsFromSpecificList(){
			...
			$arrPosts = Mage::helper('nwdrevslider')->getProductsByIDs($strPosts);
			return($arrPosts);
		}
