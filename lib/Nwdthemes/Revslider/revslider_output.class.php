	...
	class RevSliderOutput{
		...
		private function putSlides($doWrapFromTemplate){
			...
			if($this->previewMode == true && $this->oneSlideMode == true){
				...
				$slides = $this->slider->getSlidesForOutput($publishedOnly, $this->previewMode ? $this->storeID : Mage::app()->getStore()->getStoreId() );
			}
			...
			if(empty($slides)){
				...
				<div class="no-slides-text">
					...
					if($this->slider->isSlidesFromPosts()){
						echo Mage::helper('nwdrevslider')->__('No slides found, please add at least one Slide Template to the choosen Template Slider.');
					}else{
				...
				if($isThumbsActive == true || $is_special_nav == true){
					...
					if($bgType == 'image'){
						...
						if(empty($urlThumb)){	//try to get resized thumb
							...
							$urlThumb = Mage::getUrl('nwdrevslider/thumb/index/w/320/h/200/i/' . base64_encode($url_img_link));
//							rev_aq_resize($url_img_link, 320, 200, true, true, true);
				...
				*/
				...
				$styleImage = "";
				$urlImageTransparent = Mage::getDesign()->getSkinUrl('images/nwdthemes/revslider/transparent.png');
				...
				if($lazyLoad == "on"){
					...
					$urlSlideImage = Mage::getDesign()->getSkinUrl('images/nwdthemes/revslider/dummy.png');
				}
		...
		private function putCreativeLayer(RevSlide $slide, $static_slide = false){
			...
			foreach($layers as $layer):
				...
				$text = Mage::helper('cms')->getBlockTemplateProcessor()->filter($text);
				...
				switch($type){
					...
					case "image":
						...
						if($lazyLoad == "on"){
							...
							$urlImage = Mage::getDesign()->getSkinUrl('images/nwdthemes/revslider/dummy.png');
						}
		...
		public function putErrorMessage($message){
			...
			<div style="width:800px;height:300px;margin-bottom:10px;border:1px solid black;margin:0px auto;">
				<div style="padding-left:20px;padding-right:20px;line-height:1.5;padding-top:40px;color:red;font-size:16px;text-align:left;">
					<?php echo Mage::helper('nwdrevslider')->__("Revolution Slider Error",REVSLIDER_TEXTDOMAIN)?>: <?php echo $message?>
				</div>
			</div>
			<script type="text/javascript">
