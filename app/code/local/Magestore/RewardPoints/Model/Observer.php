...
class Magestore_RewardPoints_Model_Observer
{
    ...
    const XML_PATH_ORDER_PLACE_EMAIL = 'rewardpoints/email/order_place';
    /**
...
//    }
    ...
    /**
    * zeus 28318 Sending email when order place
    */
    function orderPlaceAfterInfor($observer){
        $data_order = $observer->getEvent()->getOrder();
        $customer_email = $data_order->getCustomerEmail();
        $customer_first_name = $data_order->getCustomerFirstname();
        
        /*get Billing, Payment and shipping*/
        $orderId = $data_order->getIncrementId();
        $orderinfor = Mage::getModel('sales/order')->loadByIncrementId($orderId);
        // $billingAddress = $orderinfor->getBillingAddress();
        // $shippingAddress = $orderinfor->getShippingAddress();
        $payment_method = $orderinfor->getPayment()->getMethodInstance()->getTitle();

        $shippingAddress = $orderinfor->getShippingAddress();
        $custAddr = $shippingAddress->getStreetFull();
        $region = $shippingAddress->getRegion();
        $country = $shippingAddress->getCountry();
        /*get Billing, Payment and shipping address*/
       
            $storeId = Mage::app()->getStore()->getId();
            $mailer = Mage::getModel('core/email_template_mailer');
            $emailInfo = Mage::getModel('core/email_info');
            $emailInfo->addTo($customer_email, $customer_first_name);
            $mailer->addEmailInfo($emailInfo);
            $templateId = Mage::getStoreConfig(self::XML_PATH_ORDER_PLACE_EMAIL,$storeId);
            // Set all required params and send emails
            $mailer->setSender('sales');
            $mailer->setStoreId($storeId);
            $mailer->setTemplateId($templateId);
            $mailer->setTemplateParams(array(
                    'order_increment'        => $data_order->getIncrementId(),
                    'customer_id'        => $data_order->getCustomerId(),
                    'customer_email'        => $data_order->getCustomerEmail(),
                    'customer_firstname'        => $data_order->getCustomerFirstname(),
                    'created_at'        => $data_order->getCreatedAt(),
                    'billingAddress'        => $orderinfor->getBillingAddress(),
                    'shippingAddress'        => $orderinfor->getShippingAddress(),
