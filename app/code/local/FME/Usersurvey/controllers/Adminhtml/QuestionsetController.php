...
class FME_Usersurvey_Adminhtml_QuestionsetController extends Mage_Adminhtml_Controller_Action {
    ...
    public function editAction() {
        ...
        if ($model->getId() || $id == 0) {
            ...
            Mage::register('questionset_data', $model);
    ...
    public function saveAction() {
        if ($data = $this->getRequest()->getPost()) {
			...
			if (isset($links['related'])) {
                $questionIds = Mage::helper('adminhtml/js')->decodeGridSerializedInput($links['related']);
                foreach ($questionIds as $_question) {
                    $questionString .= $_question . ",";
                }
            }

            /* echo "<pre>";
              print_r($questionString); exit(); */
            $data['stores'] = implode(",", $data['stores']);
            if ($questionString != '') {
                $data['question_ids'] = $questionString;
            } else {
                $data['question_ids'] = $data['q_ids'];
            }
    ...
    public function massDeleteAction() {
        $questionsetIds = $this->getRequest()->getParam('questionset');
        if (!is_array($questionsetIds)) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select item(s)'));
        } else {
            try {
                foreach ($questionsetIds as $questionsetId) {
                    $questionset = Mage::getModel('usersurvey/questionset')->load($questionsetId);
                    $questionset->delete();
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(
                        Mage::helper('adminhtml')->__(
                                'Total of %d record(s) were successfully deleted', count($questionsetIds)
                        )
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
    ...
    public function massStatusAction() {
        ...
        if (!is_array($questionsetIds)) {
            ...
            try {
                foreach ($questionsetIds as $questionsetId) {
                    $questionset = Mage::getSingleton('usersurvey/questionset')
                            ->load($questionsetId)
                ...
                $this->_getSession()->addSuccess(
                        $this->__('Total of %d record(s) were successfully updated', count($questionsetIds))
                );
            } catch (Exception $e) {
    ...
    public function exportCsvAction() {
        $fileName = 'questionset.csv';
        $content = $this->getLayout()->createBlock('usersurvey/adminhtml_questionset_grid')
                ->getCsv();
    ...
    public function questionsAction() {
        $this->loadLayout();
        $this->getLayout()->getBlock('questions.grid');
        $this->renderLayout();
    }
