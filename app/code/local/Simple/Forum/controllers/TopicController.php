	...
	public function saveAction()
	{
		...
		if ( $this->getRequest()->getPost() )
		{
			try
			{
				...
				if(!$topicId || ($topic->getSystem_user_id() == $system_user_id && !empty($postData['Title'])) || ($isModerator && !empty($postData['Title'])))
				{
					...
					Mage::helper('forum/topic')->setEntity($topicModel->getId(), $topicModel, true);
				 	if(!$topicId)
				 	{
	                	...
	                	Mage::helper('forum/topic')->updateUrlRewrite($id_path, Mage::app()->getStore()->getId(), 'forum/' . $url_text, $requestPath);
				...
				{
					$postData['Post'] = preg_replace('#<script(.*?)>(.*?)</script>#is', '', $postData['Post']);
					...
					Mage::helper('forum/topic')->setUserForum($postModel->getSystem_user_id(), $postModel);
					if(!$postId)
	                {
						$postModel->setCreated_time(now());
						Mage::helper('forum/topic')->updateTotalPostsUser($system_user_id);
					}
					else
					{
					...
					if( Mage::getStoreConfig('forum/forumnotification/allownotifycheckbox') )
					{
						...
						{
							if($postData['notify_me'] == 'on')Mage::helper('forum/notify')->addToNotifyList($topicId);
